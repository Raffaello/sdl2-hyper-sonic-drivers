        ...
        namespace woody
        {
            ...
            void processcell_attack(celltype* ctc, const double modulator, const double vib, const double trem)
            {
                ...
                for (int32_t ct = 0; ct < num_steps_add; ct++)
                {
                    ...
                    if ((ctc->cur_env_step & ctc->env_step_a) == 0)
            ...
            void OPLChip::change_feedback(const uint32_t chanbase, celltype* c)
            {
                ...
                if (feedback)
                    c->mfb = (double)(pow(FL2, (double)((feedback >> 1) + 5)) * (WAVPREC / 2048.0) * FL05);
                else
                    c->mfb = 0.0;
            }
                ...
                for (int32_t cursmp = 0; cursmp < samples_to_process; cursmp += endsamples)
                {
                    ...
                    for (i = 0; i < endsamples; i++)
                    {
                        ...
                        if (adlibreg[ARC_PERC_MODE] & 0x80)
                            trem_lut[i] = trem_table[(int32_t)tremtab_pos];
                        else
                            trem_lut[i] = trem_table[TREMTAB_SIZE + (int32_t)tremtab_pos];
                    }
