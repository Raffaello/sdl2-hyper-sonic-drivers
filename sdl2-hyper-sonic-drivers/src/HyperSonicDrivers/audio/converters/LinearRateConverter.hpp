...
public:
    LinearRateConverter(uint32_t in_rate, uint32_t outrate);
 ...
 */
template <bool stereo, bool reverseStereo>
LinearRateConverter<stereo, reverseStereo>::LinearRateConverter(uint32_t in_rate, uint32_t outrate)
{
    if (in_rate >= 131072 || outrate >= 131072)
    {
    ...
    // This will overflow if in_rate >= 2^17, and underflow if outrate >= 2^17.
    ...
    out_pos_inc = (in_rate << fracBitsLow) / outrate;
...
size_t LinearRateConverter<stereo, reverseStereo>::flow(IAudioStream& input, int16_t* obuf, uint32_t osamp, const uint16_t vol_l, const uint16_t vol_r)
{
    ...
    while (obuf < oend)
    {
        ...
        while (out_pos < fracOneLow && obuf < oend)
        {
            // interpolate
            const int16_t out0 = interpolate(ilast0, icur0, out_pos);
            const int16_t out1 = stereo ? interpolate(ilast1, icur1, out_pos) : out0;
            // output left channel
