    ...
    RateConverter* makeRateConverter(uint32_t inrate, uint32_t outrate) {
        if (inrate != outrate) {
            if ((inrate % outrate) == 0 && (inrate < 65536)) {
                return new converters::SimpleRateConverter<stereo, reverseStereo>(inrate, outrate);
            }
            else {
                return new converters::LinearRateConverter<stereo, reverseStereo>(inrate, outrate);
            }
        }
        else {
            return new converters::CopyRateConverter<stereo, reverseStereo>();
        }
    }
