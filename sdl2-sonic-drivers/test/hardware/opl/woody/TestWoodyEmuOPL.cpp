...
namespace hardware
{
    namespace opl
    {
        namespace woody
        {
            ...
            TEST(WoodyEmuOPL, share_ptrDefault)
            {
                std::shared_ptr<MockMixer> mixer = std::make_shared<MockMixer>();
                EXPECT_EQ(mixer.use_count(), 1);

                std::shared_ptr<WoodyEmuOPL> wOpl = std::make_shared<WoodyEmuOPL>(mixer, true);
                EXPECT_EQ(mixer.use_count(), 2);
            ...
            TEST(DISABLED_WoodyEmuOPL, Table440Hz)
            {
                std::shared_ptr<MockMixer> mixer = std::make_shared<MockMixer>();
                mixer->rate = 22050;
                std::shared_ptr<WoodyEmuOPL> opl = std::make_shared<WoodyEmuOPL>(mixer, true);
                opl->init();
