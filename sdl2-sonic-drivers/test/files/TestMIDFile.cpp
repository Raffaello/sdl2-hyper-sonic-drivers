...
namespace files
{
    ...
    void cmp_midievent(const int i, const audio::midi::MIDIEvent& e, const uint32_t exp_abs_time, const uint32_t exp_delta_time, const std::vector<uint8_t>& exp_data)
    {
        EXPECT_EQ(e.abs_time, exp_abs_time) << "index: " << i;
        EXPECT_EQ(e.delta_time, exp_delta_time) << "index: " << i;

        ASSERT_EQ(e.data.size(), exp_data.size() - 1) << "index: " << i;
        EXPECT_EQ(e.type.val, exp_data[0]) << "index: " << i;
        for (int j = 0; j < e.data.size(); j++)
            EXPECT_EQ(e.data[j], exp_data[j + 1]) << "index: " << i << ", " << j;
    }
    ...
    TEST(MIDFile, midifile_sample_convert_to_single_track)
    {
        ...
        for (int i = 0; i < 61; i++) {
            cmp_midievent(i, t.events[i], exp_abs_times[i], exp_delta_times[i], exp_datas[i]);
        }
    }
}
