...
#define ARRAYSIZE(x) ((int)(sizeof(x) / sizeof(x[0])))
	...
	void AdLibDriver::play(uint8 track, uint8 volume)
	{
		uint16 soundId = 0;
	...
	void AdLibDriver::internalLoadFile(const char* file)
	{
		...
		setSoundData(_soundDataPtr, soundDataSize);
	...
	void AdLibDriver::haltTrack() {
		play(0, 0);
		play(0, 0);
		//_vm->_system->delayMillis(3 * 60);
	}

AdLibDriver::AdLibDriver(std::shared_ptr<audio::scummvm::Mixer> mixer, int version, std::shared_ptr< hardware::opl::OPL> opl) : PCSoundDriver() {
	_version = version;
...
void AdLibDriver::setMusicVolume(uint8_t volume) {
	...
	for (uint i = 0; i < 6; ++i) {
		...
		const uint8_t regOffset = _regOffset[i];
...
void AdLibDriver::setSfxVolume(uint8_t volume) {
...
void AdLibDriver::setSoundData(uint8_t *data, uint32_t size) {
	////Common::StackLock lock(_mutex);
...
void AdLibDriver::startSound(int track, int volume) {
	...
	if (_programQueueEnd == _programQueueStart && _programQueue[_programQueueEnd].data != 0)
		debugC(3, 0, "AdLibDriver: Program queue full, dropping track %d", _programQueue[_programQueueEnd].id);
...
void AdLibDriver::setupPrograms() {
	...
	if (retrySound.data) {
		debugC(9, 0, "AdLibDriver::setupPrograms(): WORKAROUND - Restarting skipped sound %d)", retrySound.id);
		startSound(retrySound.id, retrySound.volume);
	}
}

void AdLibDriver::adjustSfxData(uint8_t *ptr, int volume) {
	...
	for (_curChannel = 9; _curChannel >= 0; --_curChannel) {
		...
		while (result == 0 && dataptr) {
			...
			if (opcode & 0x80) {
				...
				debugC(9, 0, "Calling opcode '%s' (%d) (channel: %d)", op.name, opcode, _curChannel);
				...
				debugC(9, 0, "Note on opcode 0x%02X (duration: %d) (channel: %d)", opcode, duration, _curChannel);
...
void AdLibDriver::initChannel(Channel &channel) {
	debugC(9, 0, "initChannel(%lu)", (long)(&channel - _channels));
...
void AdLibDriver::noteOff(Channel &channel) {
	debugC(9, 0, "noteOff(%lu)", (long)(&channel - _channels));
...
void AdLibDriver::initAdlibChannel(uint8_t chan) {
	debugC(9, 0, "initAdlibChannel(%d)", chan);
...
void AdLibDriver::setupDuration(uint8_t duration, Channel &channel) {
	debugC(9, 0, "setupDuration(%d, %lu)", duration, (long)(&channel - _channels));
...
void AdLibDriver::setupNote(uint8_t rawNote, Channel &channel, bool flag) {
	debugC(9, 0, "setupNote(%d, %lu)", rawNote, (long)(&channel - _channels));
...
void AdLibDriver::setupInstrument(uint8_t regOffset, const uint8_t *dataptr, Channel &channel) {
	debugC(9, 0, "setupInstrument(%d, %p, %lu)", regOffset, (const void *)dataptr, (long)(&channel - _channels));
...
void AdLibDriver::noteOn(Channel &channel) {
	debugC(9, 0, "noteOn(%lu)", (long)(&channel - _channels));
...
void AdLibDriver::adjustVolume(Channel &channel) {
	debugC(9, 0, "adjustVolume(%lu)", (long)(&channel - _channels));
...
void AdLibDriver::primaryEffectSlide(Channel &channel) {
	debugC(9, 0, "Calling primaryEffectSlide (channel: %d)", _curChannel);
...
void AdLibDriver::primaryEffectVibrato(Channel &channel) {
	debugC(9, 0, "Calling primaryEffectVibrato (channel: %d)", _curChannel);
...
void AdLibDriver::secondaryEffect1(Channel &channel) {
	debugC(9, 0, "Calling secondaryEffect1 (channel: %d)", _curChannel);
...
uint8_t AdLibDriver::calculateOpLevel1(Channel &channel) {
	...
	if (value & 0x80)
		debugC(3, 0, "AdLibDriver::calculateOpLevel1(): WORKAROUND - total level clipping uint/int bug encountered");
...
uint8_t AdLibDriver::calculateOpLevel2(Channel &channel) {
	...
	if (value & 0x80)
		debugC(3, 0, "AdLibDriver::calculateOpLevel2(): WORKAROUND - total level clipping uint/int bug encountered");
	value = CLIP<uint8_t>(value, 0, 0x3F);
...
int AdLibDriver::update_setRepeat(Channel &channel, const uint8_t *values) {
	channel.repeatCounter = values[0];
	return 0;
}

int AdLibDriver::update_checkRepeat(Channel &channel, const uint8_t *values) {
...
int AdLibDriver::update_setupProgram(Channel &channel, const uint8_t *values) {
	if (values[0] == 0xFF)
		return 0;
	...
	if (!checkDataOffset(ptr, 2)) {
		debugC(3, 0, "AdLibDriver::update_setupProgram: Invalid program %d specified", values[0]);
		return 0;
	}
...
int AdLibDriver::update_setNoteSpacing(Channel &channel, const uint8_t *values) {
	channel.spacing1 = values[0];
	return 0;
}

int AdLibDriver::update_jump(Channel &channel, const uint8_t *values) {
	int16_t add = READ_LE_UINT16(values);
	// Safety check: ignore jump to invalid address
	if (_version == 1)
...
int AdLibDriver::update_jumpToSubroutine(Channel &channel, const uint8_t *values) {
	int16_t add = READ_LE_UINT16(values);
...
int AdLibDriver::update_returnFromSubroutine(Channel &channel, const uint8_t *values) {
...
int AdLibDriver::update_setBaseOctave(Channel &channel, const uint8_t *values) {
	channel.baseOctave = values[0];
	return 0;
}

int AdLibDriver::update_stopChannel(Channel &channel, const uint8_t *values) {
	channel.priority = 0;
	if (_curChannel != 9)
		noteOff(channel);
...
int AdLibDriver::update_playRest(Channel &channel, const uint8_t *values) {
	setupDuration(values[0], channel);
	noteOff(channel);
	return values[0] != 0;
}

int AdLibDriver::update_writeAdLib(Channel &channel, const uint8_t *values) {
	writeOPL(values[0], values[1]);
	return 0;
}

int AdLibDriver::update_setupNoteAndDuration(Channel &channel, const uint8_t *values) {
	setupNote(values[0], channel);
	setupDuration(values[1], channel);
	return values[1] != 0;
}

int AdLibDriver::update_setBaseNote(Channel &channel, const uint8_t *values) {
	channel.baseNote = values[0];
	return 0;
}

int AdLibDriver::update_setupSecondaryEffect1(Channel &channel, const uint8_t *values) {
...
int AdLibDriver::update_stopOtherChannel(Channel &channel, const uint8_t *values) {
	// Safety check
	if (values[0] > 9) {
...
int AdLibDriver::update_waitForEndOfProgram(Channel &channel, const uint8_t *values) {
	const uint8_t *ptr = getProgram(values[0]);
	...
	if (!ptr) {
		debugC(3, 0, "AdLibDriver::update_waitForEndOfProgram: Invalid program %d specified", values[0]);
		return 0;
	}
...
int AdLibDriver::update_setupInstrument(Channel &channel, const uint8_t *values) {
	...
	// of the game.
	if (!instrument) {
		debugC(3, 0, "AdLibDriver::update_setupInstrument: Invalid instrument %d specified", values[0]);
		return 0;
	}
...
int AdLibDriver::update_setupPrimaryEffectSlide(Channel &channel, const uint8_t *values) {
...
int AdLibDriver::update_removePrimaryEffectSlide(Channel &channel, const uint8_t *values) {
	channel.primaryEffect = nullptr;
	channel.slideStep = 0;
	return 0;
}

int AdLibDriver::update_setBaseFreq(Channel &channel, const uint8_t *values) {
	channel.baseFreq = values[0];
	return 0;
}

int AdLibDriver::update_setupPrimaryEffectVibrato(Channel &channel, const uint8_t *values) {
...
int AdLibDriver::update_setPriority(Channel &channel, const uint8_t *values) {
...
int AdLibDriver::update_setBeat(Channel &channel, const uint8_t *values) {
...
int AdLibDriver::update_waitForNextBeat(Channel &channel, const uint8_t *values) {
...
int AdLibDriver::update_setExtraLevel1(Channel &channel, const uint8_t *values) {
	channel.opExtraLevel1 = values[0];
	adjustVolume(channel);
	return 0;
}

int AdLibDriver::update_setupDuration(Channel &channel, const uint8_t *values) {
	setupDuration(values[0], channel);
	return values[0] != 0;
}

int AdLibDriver::update_playNote(Channel &channel, const uint8_t *values) {
	setupDuration(values[0], channel);
	noteOn(channel);
	return values[0] != 0;
}

int AdLibDriver::update_setFractionalNoteSpacing(Channel &channel, const uint8_t *values) {
	channel.fractionalSpacing = values[0] & 7;
	return 0;
}

int AdLibDriver::update_setTempo(Channel &channel, const uint8_t *values) {
	_tempo = values[0];
	return 0;
}

int AdLibDriver::update_removeSecondaryEffect1(Channel &channel, const uint8_t *values) {
	channel.secondaryEffect = nullptr;
	return 0;
}

int AdLibDriver::update_setChannelTempo(Channel &channel, const uint8_t *values) {
	channel.tempo = values[0];
	return 0;
}

int AdLibDriver::update_setExtraLevel3(Channel &channel, const uint8_t *values) {
	channel.opExtraLevel3 = values[0];
	return 0;
}

int AdLibDriver::update_setExtraLevel2(Channel &channel, const uint8_t *values) {
	// Safety check
	if (values[0] > 9) {
...
int AdLibDriver::update_changeExtraLevel2(Channel &channel, const uint8_t *values) {
	// Safety check
	if (values[0] > 9) {
...
int AdLibDriver::update_setAMDepth(Channel &channel, const uint8_t *values) {
	if (values[0] & 1)
...
int AdLibDriver::update_setVibratoDepth(Channel &channel, const uint8_t *values) {
	if (values[0] & 1)
...
int AdLibDriver::update_changeExtraLevel1(Channel &channel, const uint8_t *values) {
	channel.opExtraLevel1 += values[0];
	adjustVolume(channel);
	return 0;
}

int AdLibDriver::update_clearChannel(Channel &channel, const uint8_t *values) {
	// Safety check
	if (values[0] > 9) {
...
int AdLibDriver::update_changeNoteRandomly(Channel &channel, const uint8_t *values) {
	if (_curChannel >= 9)
		return 0;
...
int AdLibDriver::update_removePrimaryEffectVibrato(Channel &channel, const uint8_t *values) {
	channel.primaryEffect = nullptr;
	return 0;
}

int AdLibDriver::update_pitchBend(Channel &channel, const uint8_t *values) {
	channel.pitchBend = (int8_t)values[0];
	setupNote(channel.rawNote, channel, true);
	return 0;
}

int AdLibDriver::update_resetToGlobalTempo(Channel &channel, const uint8_t *values) {
	channel.tempo = _tempo;
	return 0;
}

int AdLibDriver::update_nop(Channel &channel, const uint8_t *values) {
	return 0;
}

int AdLibDriver::update_setDurationRandomness(Channel &channel, const uint8_t *values) {
	channel.durationRandomness = values[0];
	return 0;
}

int AdLibDriver::update_changeChannelTempo(Channel &channel, const uint8_t *values) {
	channel.tempo = CLIP(channel.tempo + (int8_t)values[0], 1, 255);
	return 0;
}

int AdLibDriver::updateCallback46(Channel &channel, const uint8_t *values) {
...
int AdLibDriver::update_setupRhythmSection(Channel &channel, const uint8_t *values) {
	int channelBackUp = _curChannel;
	...
	if (instrument) {
		...
		debugC(3, 0, "AdLibDriver::update_setupRhythmSection: Invalid instrument %d for channel 6 specified", values[0]);
	}
	...
	if (instrument) {
		...
		debugC(3, 0, "AdLibDriver::update_setupRhythmSection: Invalid instrument %d for channel 7 specified", values[1]);
	}
	...
	if (instrument) {
		...
		debugC(3, 0, "AdLibDriver::update_setupRhythmSection: Invalid instrument %d for channel 8 specified", values[2]);
	}
...
int AdLibDriver::update_playRhythmSection(Channel &channel, const uint8_t *values) {
...
int AdLibDriver::update_removeRhythmSection(Channel &channel, const uint8_t *values) {
...
int AdLibDriver::update_setRhythmLevel2(Channel &channel, const uint8_t *values) {
...
int AdLibDriver::update_changeRhythmLevel1(Channel &channel, const uint8_t *values) {
	uint8_t ops = values[0], v = values[1];
...
int AdLibDriver::update_setRhythmLevel1(Channel &channel, const uint8_t *values) {
	uint8_t ops = values[0], v = values[1];
...
int AdLibDriver::update_setSoundTrigger(Channel &channel, const uint8_t *values) {
	_soundTrigger = values[0];
	return 0;
}

int AdLibDriver::update_setTempoReset(Channel &channel, const uint8_t *values) {
	channel.tempoReset = values[0];
	return 0;
}

int AdLibDriver::updateCallback56(Channel &channel, const uint8_t *values) {
...
const uint8_t AdLibDriver::_regOffset[] = {
...
const uint8_t *const AdLibDriver::_unkTable2[] = {
...
const uint8_t AdLibDriver::_pitchBendTables[][32] = {
	// 0
