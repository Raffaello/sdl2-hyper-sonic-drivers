...
namespace drivers
{
    namespace midi
    {
        namespace scummvm
        {
            namespace miles
            {
                using utils::ARRAYCLEAR;
                using utils::READ_LE_UINT16;
                using utils::READ_LE_UINT32;
...
#define MILES_ADLIB_STEREO_PANNING_THRESHOLD_RIGHT 100
                ...
                uint16_t milesAdLibOperator1Register[MILES_ADLIB_PHYSICAL_FMVOICES_COUNT_MAX] = {
                ...
                uint16_t milesAdLibOperator2Register[MILES_ADLIB_PHYSICAL_FMVOICES_COUNT_MAX] = {
                ...
                uint16_t milesAdLibChannelRegister[MILES_ADLIB_PHYSICAL_FMVOICES_COUNT_MAX] = {
                ...
                // hardcoded, dumped from ADLIB.MDI
                uint16_t milesAdLibVolumeSensitivityTable[] = {
                    82, 85, 88, 91, 94, 97, 100, 103, 106, 109, 112, 115, 118, 121, 124, 127
                };

                // MIDI panning to register volume table for dual OPL2
                // hardcoded, dumped from ADLIB.MDI
                uint8_t milesAdLibPanningVolumeLookUpTable[] = {
                ...
                MidiDriver_Miles_AdLib::MidiDriver_Miles_AdLib(std::shared_ptr<InstrumentEntry> instrumentTablePtr, uint16_t instrumentTableCount)
                    ...
                    _oplType = hardware::opl::scummvm::Config::OplType::OPL3;
                ...
                int MidiDriver_Miles_AdLib::open() {
                    ...
                    hardware::opl::TimerCallBack cb = std::bind(&MidiDriver_Miles_AdLib::onTimer, this);
                ...
                void MidiDriver_Miles_AdLib::setVolume(uint8_t volume) {
                ...
                void MidiDriver_Miles_AdLib::resetData() {
                    ...
                    for (uint8_t midiChannel = 0; midiChannel < MIDI_CHANNEL_COUNT; midiChannel++) {
                ...
                void MidiDriver_Miles_AdLib::resetAdLib() {
                    ...
                    setRegister(0x01, _oplType == Config::OplType::OPL3 ? 0 : 0x20);
                    if (_oplType == Config::OplType::OPL3)
                        setRegister(0x101, 0);
                    // Timer control
                    setRegister(0x02, 0);
                ...
                void MidiDriver_Miles_AdLib::resetAdLibOperatorRegisters(uint8_t baseRegister, uint8_t value) {
                    uint8_t physicalFmVoice = 0;
                ...
                void MidiDriver_Miles_AdLib::resetAdLibFMVoiceChannelRegisters(uint8_t baseRegister, uint8_t value) {
                    uint8_t physicalFmVoice = 0;
                ...
                void MidiDriver_Miles_AdLib::send(uint32_t b) {
                    uint8_t command = b & 0xf0;
                    uint8_t channel = b & 0xf;
                    uint8_t op1 = (b >> 8) & 0xff;
                    uint8_t op2 = (b >> 16) & 0xff;
                    ...
                    case 0xe0:
                        ...
                        break;
                    case 0xf0: // SysEx
                        spdlog::warn("MILES-ADLIB: SysEx: {#x}", b);
                        break;
                    default:
                        spdlog::warn("MILES-ADLIB: Unknown event {#02x}", command);
                    }
                }

                void MidiDriver_Miles_AdLib::send(int8_t source, uint32_t b) {
                    // TODO Implement proper multisource support.
                    if (source == -1 || source == 0)
                        send(b);
                }

                void MidiDriver_Miles_AdLib::stopAllNotes(uint8_t source, uint8_t channel) {
                ...
                uint32_t MidiDriver_Miles_AdLib::property(int prop, uint32_t param) {
                    switch (prop) {
                ...
                void MidiDriver_Miles_AdLib::applySourceVolume(uint8_t source) {
                    if (!(source == 0 || source == 0xFF))
                        return;
                ...
                int16_t MidiDriver_Miles_AdLib::searchFreeVirtualFmVoiceChannel() {
                    for (uint8_t virtualFmVoice = 0; virtualFmVoice < _modeVirtualFmVoicesCount; virtualFmVoice++) {
                ...
                int16_t MidiDriver_Miles_AdLib::searchFreePhysicalFmVoiceChannel() {
                    if (!circularPhysicalAssignment) {
                        // Older assign logic
                        for (uint8_t physicalFmVoice = 0; physicalFmVoice < _modePhysicalFmVoicesCount; physicalFmVoice++) {
                        ...
                        uint8_t physicalFmVoice = circularPhysicalAssignmentFmVoice;
                        for (uint8_t physicalFmVoiceCount = 0; physicalFmVoiceCount < _modePhysicalFmVoicesCount; physicalFmVoiceCount++) {
                            physicalFmVoice++;
                ...
                void MidiDriver_Miles_AdLib::noteOn(uint8_t midiChannel, uint8_t note, uint8_t velocity) {
                    ...
                    if (!instrumentPtr) {
                        spdlog::warn("MILES-ADLIB: noteOn: invalid instrument");
                        return;
                    }
                    ...
                    int16_t virtualFmVoice = searchFreeVirtualFmVoiceChannel();
                    ...
                    int16_t physicalFmVoice = searchFreePhysicalFmVoiceChannel();
                ...
                void MidiDriver_Miles_AdLib::noteOff(uint8_t midiChannel, uint8_t note) {
                ...
                void MidiDriver_Miles_AdLib::prioritySort() {
                    uint8_t   virtualFmVoice = 0;
                    uint16_t virtualPriority = 0;
                    uint16_t virtualPriorities[MILES_ADLIB_VIRTUAL_FMVOICES_COUNT_MAX];
                    uint16_t virtualFmVoicesCount = 0;
                    ...
                    for (virtualFmVoice = 0; virtualFmVoice < _modeVirtualFmVoicesCount; virtualFmVoice++) {
                        if (_virtualFmVoices[virtualFmVoice].inUse) {
                            ...
                            uint8_t currentActiveVoicesCount = _midiChannels[midiChannel].currentActiveVoicesCount;
                    ...
                    while (virtualFmVoicesCount) {
                        uint16_t unvoicedHighestPriority = 0;
                        uint8_t   unvoicedHighestFmVoice = 0;
                        uint16_t voicedLowestPriority = 65535;
                        uint8_t   voicedLowestFmVoice = 0;
                        ...
                        uint8_t physicalFmVoice = _virtualFmVoices[voicedLowestFmVoice].physicalFmVoice;
                ...
                void MidiDriver_Miles_AdLib::releaseFmVoice(uint8_t virtualFmVoice) {
                    ...
                    uint8_t midiChannel = _virtualFmVoices[virtualFmVoice].actualMidiChannel;
                    uint8_t physicalFmVoice = _virtualFmVoices[virtualFmVoice].physicalFmVoice;
                ...
                void MidiDriver_Miles_AdLib::releaseSustain(uint8_t midiChannel) {
                ...
                void MidiDriver_Miles_AdLib::updatePhysicalFmVoice(uint8_t virtualFmVoice, bool keyOn, uint16_t registerUpdateFlags) {
                    uint8_t midiChannel = _virtualFmVoices[virtualFmVoice].actualMidiChannel;
                    ...
                    uint16_t op1Reg = milesAdLibOperator1Register[physicalFmVoice];
                    uint16_t op2Reg = milesAdLibOperator2Register[physicalFmVoice];
                    uint16_t channelReg = milesAdLibChannelRegister[physicalFmVoice];

                    uint16_t compositeVolume = 0;
                    uint8_t leftVolume = 0;
                    uint8_t rightVolume = 0;

                    if (registerUpdateFlags & kMilesAdLibUpdateFlags_Reg_40) {
                        // Calculate new volume
                        uint8_t midiVolume = _midiChannels[midiChannel].currentVolume;
                        uint8_t midiVolumeExpression = _midiChannels[midiChannel].currentVolumeExpression;
                        ...
                        if (_userVolumeScaling) {
                            if (_userMute) {
                                ...
                                uint16_t userVolume = (_sources[0].type == SOURCE_TYPE_SFX ? _userSfxVolume : _userMusicVolume); // Treat SOURCE_TYPE_UNDEFINED as music
                                compositeVolume = (compositeVolume * userVolume) >> 8;
                            }
                        }
                        // Source volume scaling might clip volume, so reduce to maximum.
                        compositeVolume = std::min(compositeVolume, (uint16_t)0x7F);

                        if (_oplType == Config::OplType::DUAL_OPL2) {
                            ...
                            uint8_t invertedPanning = 0 - (_midiChannels[midiChannel].currentPanning - 127);
                    ...
                    if (registerUpdateFlags & kMilesAdLibUpdateFlags_Reg_20) {
                        ...
                        uint8_t reg20op1 = instrumentPtr->reg20op1;
                        uint8_t reg20op2 = instrumentPtr->reg20op2;
                    ...
                    if (registerUpdateFlags & kMilesAdLibUpdateFlags_Reg_40) {
                        ...
                        uint8_t reg40op1 = instrumentPtr->reg40op1;
                        uint8_t reg40op2 = instrumentPtr->reg40op2;

                        uint16_t volumeOp1 = (~reg40op1) & 0x3F;
                        uint16_t volumeOp2 = (~reg40op2) & 0x3F;
                        ...
                        if (_oplType != Config::OplType::DUAL_OPL2) {
                            ...
                            uint8_t volumeLeftOp1 = volumeOp1;
                            uint8_t volumeRightOp1 = volumeOp1;
                            ...
                            uint8_t volumeLeftOp2 = (volumeOp2 * leftVolume) / 127;
                            uint8_t volumeRightOp2 = (volumeOp2 * rightVolume) / 127;
                            ...
                            uint8_t reg40op1left = (reg40op1 & 0xC0) | volumeLeftOp1; // keep "scaling level" and merge in our volume
                            uint8_t reg40op1right = (reg40op1 & 0xC0) | volumeRightOp1;
                            uint8_t reg40op2left = (reg40op2 & 0xC0) | volumeLeftOp2;
                            uint8_t reg40op2right = (reg40op2 & 0xC0) | volumeRightOp2;
                    ...
                    if (registerUpdateFlags & kMilesAdLibUpdateFlags_Reg_60) {
                        ...
                        uint8_t reg60op1 = instrumentPtr->reg60op1;
                        uint8_t reg60op2 = instrumentPtr->reg60op2;
                        uint8_t reg80op1 = instrumentPtr->reg80op1;
                        uint8_t reg80op2 = instrumentPtr->reg80op2;
                    ...
                    if (registerUpdateFlags & kMilesAdLibUpdateFlags_Reg_E0) {
                        // Waveform Select
                        uint8_t regE0op1 = instrumentPtr->regE0op1;
                        uint8_t regE0op2 = instrumentPtr->regE0op2;
                    ...
                    if (registerUpdateFlags & kMilesAdLibUpdateFlags_Reg_C0) {
                        // Feedback / Algorithm
                        uint8_t regC0 = instrumentPtr->regC0;

                        if (_oplType == Config::OplType::OPL3) {
                            // Panning for OPL3
                            uint8_t panning = _midiChannels[midiChannel].currentPanning;
                    ...
                    if (registerUpdateFlags & kMilesAdLibUpdateFlags_Reg_A0) {
                        ...
                        if (!keyOn) {
                            // turn off note
                            uint8_t regB0 = _physicalFmVoices[physicalFmVoice].currentB0hReg & 0x1F; // remove bit 5 "key on"
                            setRegister(0xB0 + channelReg, regB0);

                        }
                        else {
                            // turn on note, calculate frequency, octave...
                            int16_t pitchBender = _midiChannels[midiChannel].currentPitchBender;
                            uint8_t  pitchRange = _midiChannels[midiChannel].currentPitchRange;
                            int16_t currentNote = _virtualFmVoices[virtualFmVoice].currentNote;
                            int16_t physicalNote = 0;
                            int16_t halfTone = 0;
                            ...
                            uint8_t regA0 = frequency & 0xFF;
                            uint8_t regB0 = ((frequency >> 8) & 0x03) | (octave << 2) | 0x20;
                ...
                void MidiDriver_Miles_AdLib::controlChange(uint8_t midiChannel, uint8_t controllerNumber, uint8_t controllerValue) {
                    uint16_t registerUpdateFlags = kMilesAdLibUpdateFlags_None;
                ...
                void MidiDriver_Miles_AdLib::programChange(uint8_t midiChannel, uint8_t patchId) {
                    const InstrumentEntry* instrumentPtr = nullptr;
                    uint8_t patchBank = _midiChannels[midiChannel].currentPatchBank;
                    ...
                    if (!instrumentPtr) {
                        spdlog::warn("MILES-ADLIB: unknown instrument requested ({}, {})", patchBank, patchId);
                        return;
                    }
                ...
                void MidiDriver_Miles_AdLib::pitchBendChange(uint8_t midiChannel, uint8_t parameter1, uint8_t parameter2) {
                    ...
                    if ((parameter1 & 0x80) || (parameter2 & 0x80)) {
                        spdlog::warn("MILES-ADLIB: invalid pitch bend change");
                        return;
                    }
                ...
                void MidiDriver_Miles_AdLib::metaEvent(int8_t source, uint8_t type, uint8_t* data, uint16_t length) {
                    if (type == MIDI_META_END_OF_TRACK && source >= 0)
                        // Stop hanging notes and release resources used by this source.
                        deinitSource(source);
                }

                void MidiDriver_Miles_AdLib::deinitSource(uint8_t source) {
                    if (!(source == 0 || source == 0xFF))
                        return;
                ...
                void MidiDriver_Miles_AdLib::applyControllerDefaults(uint8_t source) {
                    if (!(source == 0 || source == 0xFF))
                        return;
                ...
                void MidiDriver_Miles_AdLib::setRegister(int reg, int value) {
                    if (!(reg & 0x100) || _oplType == Config::OplType::DUAL_OPL2) {
                ...
                void MidiDriver_Miles_AdLib::setRegisterStereo(uint8_t reg, uint8_t valueLeft, uint8_t valueRight) {
                    _opl->write(0x220, reg);
...
/*
                ...
                MidiDriver_Multisource* MidiDriver_Miles_AdLib_create()
                {
                    ...
                    uint8_t* streamDataPtr = nullptr;

                    uint8_t curBankId = 0;
                    uint8_t curPatchId = 0;

                    InstrumentEntry* instrumentTablePtr = nullptr;
                    uint16_t           instrumentTableCount = 0;
                    InstrumentEntry* instrumentPtr = nullptr;
                    uint32_t           instrumentOffset = 0;
                    uint16_t           instrumentDataSize = 0;
                    ...
                    while (1) {
                        if (fileDataLeft < 6)
                            spdlog::error("MILES-ADLIB: unexpected EOF in instrument file");
                    ...
                    if (instrumentTableCount == 0)
                        spdlog::error("MILES-ADLIB: no instruments in instrument file");
                    ...
                    while (1) {
                        ...
                        if (instrumentDataSize != 14)
                            spdlog::error("MILES-ADLIB: unsupported instrument size");
