...
#include <drivers/midi/scummvm/MidiDriver_NULL_Multisource.hpp>
...
namespace drivers
{
    namespace midi
    {
        namespace scummvm
        {
            const uint8_t MidiDriver_Multisource::MAXIMUM_SOURCES;
            const uint16_t MidiDriver_Multisource::DEFAULT_SOURCE_NEUTRAL_VOLUME;
            const uint16_t MidiDriver_Multisource::FADING_DELAY;
            ...
            void MidiDriver_Multisource::send(uint32_t b) {
                send(-1, b);
            }

            uint32_t MidiDriver_Multisource::property(int prop, uint32_t param) {
                switch (prop) {
            ...
            void MidiDriver_Multisource::startFade(uint16_t duration, uint16_t targetVolume) {
                for (int i = 0; i < MAXIMUM_SOURCES; ++i) {
                    startFade(i, duration, targetVolume);
                }
            }

            void MidiDriver_Multisource::startFade(uint8_t source, uint16_t duration, uint16_t targetVolume) {
                //Common::StackLock lock(_fadingMutex);
                const std::lock_guard<std::mutex> lock(_fadingMutex);
            ...
            void MidiDriver_Multisource::abortFade(uint8_t source, FadeAbortType abortType) {
                //Common::StackLock lock(_fadingMutex);
                const std::lock_guard<std::mutex> lock(_fadingMutex);
            ...
            bool MidiDriver_Multisource::isFading(uint8_t source) {
                assert(source < MAXIMUM_SOURCES);
            ...
            void MidiDriver_Multisource::setControllerDefault(ControllerDefaultType type, int16_t value) {
                ...
                case CONTROLLER_DEFAULT_PITCH_BEND_SENSITIVITY:
                    ...
                    break;
                default:
                    spdlog::warn("MidiDriver_Multisource::setControllerDefault - Unknown controller default type {}", type);
                    break;
                }
            }
            ...
            void MidiDriver_Multisource::deinitSource(uint8_t source) {
            ...
            void MidiDriver_Multisource::setSourceType(uint8_t source, SourceType type) {
                assert(source < MAXIMUM_SOURCES);
            ...
            void MidiDriver_Multisource::setSourceVolume(uint16_t volume) {
                for (int i = 0; i < MAXIMUM_SOURCES; ++i) {
                    setSourceVolume(i, volume);
                }
            }

            void MidiDriver_Multisource::setSourceVolume(uint8_t source, uint16_t volume) {
                assert(source < MAXIMUM_SOURCES);
            ...
            void MidiDriver_Multisource::resetSourceVolume(uint8_t source) {
                assert(source < MAXIMUM_SOURCES);

                setSourceVolume(source, _sources[source].neutralVolume);
            }

            void MidiDriver_Multisource::setSourceNeutralVolume(uint16_t volume) {
                for (int i = 0; i < MAXIMUM_SOURCES; ++i) {
                    setSourceNeutralVolume(i, volume);
                }
            }

            void MidiDriver_Multisource::setSourceNeutralVolume(uint8_t source, uint16_t volume) {
                assert(source < MAXIMUM_SOURCES);
