...
namespace HyperSonicDrivers::audio::scummvm
{
    ...
    public:
         ...
         */
        Timestamp(unsigned int msecs = 0, unsigned int framerate = 1);
         ...
         * @param framerate  Number of frames per second (must be > 0).
         */
        Timestamp(unsigned int secs, unsigned int frames, unsigned int framerate);
         ...
         */
        Timestamp convertToFramerate(unsigned int newFramerate) const;
        ...
        inline unsigned int framerate() const { return _framerate / _framerateFactor; }
